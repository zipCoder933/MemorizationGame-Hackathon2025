...
func _physics_process(delta: float) -> void:
	...
	#rotation.y = lerp_angle(rotation.y, atan2(linear_velocity.x, linear_velocity.z), ROTATION_LERP_SPEED)
	...
	linear_velocity.x = forward.x * (movement.z * SPEED * delta)
	linear_velocity.z = forward.z * (movement.z * SPEED * delta)
